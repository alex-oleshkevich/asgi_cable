...
class URL:
    def __init__(
            self, url: str = "", scope: Scope = None, **components: t.Any
    ) -> None:
        if scope is not None:
            ...
            if host_header is not None:
                ...
                host, port = server
                default_port = \
                    {"http": 80, "https": 443, "ws": 80, "wss": 443}[scheme]
    ...
    def remove_query_params(
            self, keys: t.Union[str, t.Sequence[str]]
    ) -> "URL":
...
class URLPath(str):
    ...
    def __new__(cls, path: str, protocol: str = "",
                host: str = "") -> "URLPath":
...
class CommaSeparatedStrings(Sequence):
    def __init__(self, value: t.Union[str, t.Sequence[str]]):
...
class ImmutableMultiDict(t.Mapping):
    def __init__(
        ...
        if not value:
            _items = []  # type: t.List[t.Tuple[t.Any, t.Any]]
        elif hasattr(value, "multi_items"):
            value = t.cast(ImmutableMultiDict, value)
            _items = list(value.multi_items())
        elif hasattr(value, "items"):
            value = t.cast(t.Mapping, value)
            _items = list(value.items())
        else:
            value = t.cast(
                t.List[t.Tuple[t.Any, t.Any]], value
            )
            _items = list(value)
...
class MultiDict(ImmutableMultiDict):
    ...
    def pop(self, key: t.Any, default: t.Any = None) -> t.Any:
        self._list = [(k, v) for k, v in self._list if k != key]
    ...
    def update(
        ...
        existing_items = [(k, v) for (k, v) in self._list if
                          k not in value.keys()]
...
class Headers(t.Mapping[str, str]):
    ...
    def __init__(
            ...
            raw: t.List[t.Tuple[bytes, bytes]] = None,
            scope: Scope = None,
    ) -> None:
        self._list = []  # type: t.List[t.Tuple[bytes, bytes]]
    ...
    def keys(self) -> t.List[str]:  # type: ignore
        return [key.decode("latin-1") for key, value in self._list]

    def values(self) -> t.List[str]:  # type: ignore
        return [value.decode("latin-1") for key, value in self._list]

    def items(self) -> t.List[t.Tuple[str, str]]:  # type: ignore
        return [
